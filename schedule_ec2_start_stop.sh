...
#
# Description:
#   Automates the scheduling of an Amazon EC2 instance to start and stop using
...
#
# Usage:
#   ./schedule_ec2_start_stop.sh -p <aws_profile> -r <aws_region> -i <instance_id>
#   ./schedule_ec2_start_stop.sh -D -p <aws_profile> -r <aws_region> -i <instance_id>
#
# Parameters:
#   -p | --profile      AWS CLI profile to use
#   -r | --region       AWS region to operate in
#   -i | --instance-id  EC2 instance identifier to manage
...
START_ASSOCIATION_NAME_PREFIX="StartEC2Instance"

# Schedule times (hour in UTC)
START_HOUR=6    # 8 in Italy
STOP_HOUR=16    # 18 in Italy
...
validate_ec2_instance() {
    ...
    if ! aws ec2 describe-instances \
        ...
        echo "Error: EC2 instance '$instance_id' not found"
        exit 1
    fi
}
...
print_help() {
  ...
  echo "  -i | --instance-id  EC2 instance identifier to manage"
  echo "  -D | --delete       Delete the created associations and IAM resources"
  echo "  -h | --help         Display this help message"
  echo
  echo "Examples:"
  echo "  $0 -p myprofile -r eu-central-1 -i i-0123456789abcdef0"
  echo "  $0 -D -p myprofile -r eu-central-1 -i i-0123456789abcdef0"
  exit 1
}
...
if [ -z "$INSTANCE_ID" ]; then
  echo "Error: EC2 instance identifier not specified. Use -i or --instance-id to set it."
  exit 1
fi
...
validate_ec2_instance "$AWS_PROFILE" "$AWS_REGION" "$INSTANCE_ID"

echo "Starting EC2 start/stop scheduling script..."
...
if [ "$DELETE_MODE" = true ]; then
  ...
  echo "Deletion of EC2 start/stop scheduling resources complete."
  exit 0
fi
...
for DAY in "${DAYS[@]}"; do
  ...
  manage_association "create" "$START_ASSOCIATION_NAME" "AWS-StartEC2Instance" "$START_SCHEDULE" "$PARAMETERS" ""

  # Create or Update Stop Association
  echo "Setting up automatic stop on $DAY with State Manager..."
  manage_association "create" "$STOP_ASSOCIATION_NAME" "AWS-StopEC2Instance" "$STOP_SCHEDULE" "$PARAMETERS" ""

done

echo "EC2 start/stop scheduling setup complete."
